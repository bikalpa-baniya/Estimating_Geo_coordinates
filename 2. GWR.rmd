...
```{r,include=FALSE}
df <- read_csv("Output/survey_w_cord.csv")
#setwd("Processing⁩/GWR_modeling")
```
...
ggwr.basic1<-function(formula, data, regression.points, bw, family ="poisson", kernel="bisquare",
...
#| HH7A =="BO"
#|  HH7A == "KAMBIA" | HH7A == "KOINADUGU" | HH7A == "PORT LOKO"
#|HH7A == "KENEMA" | HH7A == "KONO" | HH7A == "BOMBALI"  |  HH7A == "KAMBIA" | HH7A == "KOINADUGU"


df <- df%>%
  # filter(HH7A =="KAILAHUN"  )%>%
  filter(MT12!="NO RESPONSE" & MT2 != "NO RESPONSE"  #Phone and radio
         & WB6A != "NA" & WB6A != "DK" #Education
         & MA1 != "NO RESPONSE"   #Married 
         & disability != "NA")%>% #disability   
  mutate(phone_use = (MT12 == "ALMOST EVERY DAY" | MT12== "AT LEAST ONCE A WEEK"), 
         radio_use = (MT2 == "ALMOST EVERY DAY" | MT2== "AT LEAST ONCE A WEEK"),
         WB6A_Higher = (WB6A == "HIGHER"), 
          WB6A_Junior_sec = (WB6A == "JUNIOR SECONDARY"),
          WB6A_Primary = (WB6A == "PRIMARY"),
         WB6A_Senior_sec = (WB6A == "SENIOR SECONDARY"),
         WB6A_Voc = (WB6A == "VOCATION/TECHNICAL/NURSING/TEACHING"),
         MA1_Married = (MA1 != "NO, NOT IN UNION"),
         CM1_given_birth = (CM1 == "YES"),
         DV1_dom_violence = (DV1A == "YES"|DV1B == "YES"|DV1C == "YES"|DV1D == "YES"|DV1E == "YES"),
         disability_dummy  = (disability == "Has functional difficulty"),
         women_TF = (women ==1))%>%
  dplyr::select(phone_use,radio_use
         ,WB6A_Higher,WB6A_Junior_sec,WB6A_Primary,WB6A_Senior_sec,WB6A_Voc
         ,MA1_Married  
         ,CM1_given_birth
         ,DV1_dom_violence
         ,disability_dummy
...
xy <- df[,c("xcord_estimate","ycord_estimate")]
...
print(unique(df$phone_use))
print(unique(df$WB6A_Voc))

gwr_formula <- phone_use~women_TF+WB6A_Junior_sec+WB6A_Primary+WB6A_Senior_sec+MA1_Married+CM1_given_birth+DV1_dom_violence +disability_dummy+women_TF  +WB6A_Voc

#gwr_formula <- phone_use~women_TF+WB6A_Junior_sec+WB6A_Primary+WB6A_Senior_sec+MA1_Married+CM1_given_birth+DV1_dom_violence +disability_dummy+women_TF  +WB6A_Voc
# +WB6A_Voc +WB6A_Higher
...
```{r}
#a pre-specified distance matrix between regression points and observations
...
bw.f2 <- bw.ggwr(gwr_formula ,data=spdf, dMat=DM,family ="binomial") 


res.binomial<-ggwr.basic(gwr_formula , bw=bw.f2,data=spdf, dMat=DM, family ="binomial")
